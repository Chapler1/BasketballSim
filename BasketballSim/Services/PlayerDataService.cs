...
public class PlayerDataService(IWebHostEnvironment env, Nba2kCacheService nba2k)
{
    ...
    private string SpPath => Path.Combine(env.ContentRootPath, "Data", "seasons_played.json");

    // Mirror the Python normalize_name: lowercase, strip diacritics, remove . ' -
    private static string NormalizeForLookup(string name)
    {
        var nfd = name.Normalize(System.Text.NormalizationForm.FormD);
        var stripped = new string(nfd.Where(c => System.Globalization.CharUnicodeInfo.GetUnicodeCategory(c)
    ...
    private void ApplySeasonsPlayedFile(PlayerDb db)
    {
        ...
        Dictionary<string, int> spData;
        try
        {
            spData = JsonSerializer.Deserialize<Dictionary<string, int>>(
                File.ReadAllText(spPath),
