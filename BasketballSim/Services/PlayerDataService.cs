...
public class PlayerDataService(IWebHostEnvironment env, Nba2kCacheService nba2k)
{
    ...
    private string HistoricalPath => Path.Combine(env.ContentRootPath, "Data", "historical_player_stats.json");

    // Mirror Python normalize_name: lowercase, strip diacritics, remove . ‘ -
    internal static string NormalizeForLookup(string name)
    {
    ...
    // Reads historical_player_stats.json (generated by pull_nba_stats.py, never modified by sim).
    // Counts array length per player name = real NBA seasons played.
    // Players absent from the file = no real NBA history = SeasonsPlayed 0 = rookie.
    private void ApplySeasonsPlayedFile(PlayerDb db)
    {
        var path = HistoricalPath;
        ...
        try
        {
            ...
            foreach (var prop in doc.RootElement.EnumerateObject())
            {
                ...
                var key = NormalizeForLookup(prop.Name);
                int n = prop.Value.GetArrayLength();
                if (counts.TryGetValue(key, out int existing))
                    counts[key] = Math.Max(existing, n);
                else
                    counts[key] = n;
            }
        }
        ...
        foreach (var record in db.Players)
        {
            ...
            record.SeasonsPlayed = counts.TryGetValue(key, out int n) ? n : 0;
        }
    }
    ...
    private PlayerDb Load()
    {
        ...
        // Apply real NBA career season counts from seasons_played.json (generated by pull_nba_stats.py).
        // Run on every load — idempotent for veterans (Math.Max preserves sim increments).
        // Players absent from the file have no real NBA history → SeasonsPlayed = 0 → rookies.
        ApplySeasonsPlayedFile(db);
